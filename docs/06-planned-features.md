...
- **5.2 Catalog summarization + relevance review** *(active).* AI extracts requirements from the listing, produces a strategy summary, and maps catalog entries to specific requirements. Review is a three-screen wizard: (1) strategy & requirements triage — accept/reject requirements with match counts; (2) per-requirement review — one requirement per page, catalog selections with include/exclude, catalog search, freeform text entry with modal extraction; (3) confirm & generate — summary of all decisions, submit to advance to `drafting`. The freeform text entry on Screen 2 feeds back into the catalog through the existing extraction pipeline, so the catalog grows as a side effect of applying to jobs.
