...
- **4.5 Duplicate detection & merge UI** — pending. When viewing a draft, check whether one or more similar records already exist. Match rules: case-insensitive substring match against organizations (bidirectional), exact title match within an org for positions, case-insensitive substring match within an org for projects (bidirectional). Accomplishments aren't scoped by this slice. When candidates exist, the review page surfaces a "Merge into..." action alongside Confirm/Reject. The merge UI is a side-by-side editor showing existing vs draft values per field, with a "Use this" button under each value as the chooser. Textarea fields get a third column for AI-synthesized combined text, fetched on demand via `ExtractionProvider::synthesize()`. When more than one candidate matches, the user picks one before the editor opens. On merge: the target record is updated with chosen values, the draft is marked `status='merged'` with `match_record_type`/`match_record_id` pointing at the target, and pending dependent drafts in the same source document have their parent-name references rewritten to the target's canonical name so they continue resolving at confirmation time.
