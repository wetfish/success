...
This section is transitional documentation that exists to guide the initial creation of migration files. Once migrations are written, the migration files themselves are the authoritative ordering reference and this section should be removed.

When generating Laravel migrations, the order matters because foreign keys can only point at tables that already exist. Suggested order:
...
1. `people`
1. `positions`
1. `projects` (self-referential FK is added in this same migration since it points back at the same table)
1. `accomplishments`
1. `source_documents`
1. `career_themes`
1. `links` (polymorphic, no DB FK constraints)
1. `taggables` (polymorphic, no DB FK constraints)
1. `accomplishment_collaborators`
1. `accomplishment_source_documents`
1. `project_source_documents`
1. `career_theme_projects`
1. `career_theme_accomplishments`

Per the AI development notes, when generating multiple migrations in sequence, add `sleep 1` between artisan commands to ensure unique timestamps.